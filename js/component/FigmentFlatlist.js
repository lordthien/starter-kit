...
/**
 * Flatlist wrapper that encapsulates behavior for the Flatlist seen at the bottom of the screen
 * in the app.
 */
class FigmentFlatlist extends Component {
  constructor(props) {
    super(props);

    // The app was written before React-Native's Flatlist was ready for prime time
    // Flatlist necessitates relying on componentWillReceiveProps, which since updating
  ...
  // Check if given rowId in the Flatlist is selected, used to render the pink border around chosen effect
  ...
  // Called when animation on the FlatlistItem is done
  _onAnimationDone() {
    this.setState({
      animationDone: true,
    });
...
module.exports = connect(selectProps)(FigmentFlatlist);
